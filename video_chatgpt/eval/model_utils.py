...
def load_frames(vis_path, n_clips=1, num_frm=1):
    frame_fnames = sorted(os.listdir(vis_path))
    total_num_frm = min(num_frm, len(frame_fnames))
    frame_idx = get_seq_frames(len(frame_fnames), total_num_frm)

    img_array = np.array([Image.open(os.path.join(vis_path, frame_fnames[f_idx])) for f_idx in frame_idx])
